...
{
    public sealed class EncryptIdRewriteConverter(List<string> encryptedProperties, EncryptionConfigurationModel configuration) : Newtonsoft.Json.JsonConverter
    {
        ...
        private void WriteSingleValue(Newtonsoft.Json.JsonWriter writer, long value)
        {
            if (encryptedProperties.Contains(GetJsonPath(writer.Path)))
            {
                var encryptionConfig = configuration?.EncryptionConfigurations?.FirstOrDefault(x => x.Name == "IdEncryption");

                writer.WriteValue(EncryptionHelper.EncryptNumeric(value, encryptionConfig?.Key ?? "", encryptionConfig?.IV ?? ""));
            }
            else
            {
